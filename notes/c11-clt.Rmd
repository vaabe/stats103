...
- it can be approximated by a normal curve when provided $n$ is large  
...
This can be used to calculate probabilities associated with any observed sample mean for a given experiment.  

Of course, in most experiments we do not know the population mean and variance. But this does not forbid us from using the CLT---it turns out we can simply substitute our sample-derived estimates for mean and variance instead, as **plug-in estimates**.  
...
Now you can simply use `pnorm()` to make probability calculations.  

E.g. the probability that the mean hourly wage gap percent is less than 10.5:

```{r}
pnorm(10.5, mean = Xbar, sd = SE)
```

E.g. the probability that the mean hourly wage gap percent is between 10 and 13:
...
To be strictly technical, you should always use the DoF-adjusted formula any time you are using sample data. But in reality, when $n$ is large, the distinction is only important for small samples. Note that in R, the `sd()` function uses the DoF-adjustment formula by default.  

## The t-distribution 

When sample size is small, you should use the $t$-distribution to approximate the limiting behavior of the sample mean (as opposed to the normal distribution). The $t$-distribution is designed to handle sample distributions with a small number of observations ($n < 30$).  
...
Note that for $\text{DoF}=30$, the $t$-distribution is almost exactly the same as the normal curve. The difference between the two curves is only significant for small $n$. Thus if your sample is large enough, you needn't worry about the using $t$-distribution---it only becomes important for small samples.  
