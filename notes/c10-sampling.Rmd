...
Compare these sample means to the theoretical mean of the population, in this case $\mu = 3.5$. Clearly, the error varies substantially across the two observed samples.^[Here error refers to the difference between the true and sample values of a parameter.]

This a trivial example, since the RV in question follows a known distribution (discrete uniform), and we thus know the population mean exactly. In most experimental contexts the population distribution is unknown---all we have is a sample distribution, and whatever estimates it produces.   
...
Reliability refers to the _consistency_ of an estimator across different samples. An estimator with high variability---e.g. the sample mean in the "diceroll" simulation above---is said to be *unreliable*, as its value tends to change significantly across samples.  
...
In the "diceroll" simulation above we used a sample size $n = 10$. Below we rerun the same simulation three times, but with sample sizes $n = \{ 30, 1000, 10000 \}$. The sample mean is computed for each value of $n$.  
...
The **weak law of large numbers** says the sample mean converges *in probability* to the true mean for large $n$. Specifically, it says the probability that the sampling error is larger than some nonzero margin goes to zero for large $n$:
...
These theorems are immensely useful for characterizing sample variability, and they have many applications in the statistical methods introduced in module I.  
