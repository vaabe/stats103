...
A **population** is the relevant set of observations for conducting an experiment or answering a question fully---e.g. in the pay gap study, the target population might be the salary of *every* person in the UK. It's often unrealistic (or impossible) to get data on a whole population, and samples are thus used instead.  

A **sample** is a subset of data from a population---e.g. the pay gap dataset is a *sample* of income data for 10,000 UK firms. Samples are used to represent populations, and make inferences about population characteristics which are often unknown. The **sample size** (i.e. the number of observations) is typically denoted $n$.  
...
```{r, echo=FALSE}
parameters = data.frame("parameter" = c("mean","standard deviation","correlation coefficient"), "population" = c("$\\mu$","$\\sigma$","$\\rho$"), "sample" = c("$\\bar x$","$s$","$r$"))
...
Since every sample of data is unique, the estimates derived therefrom will vary across different samples. A truly random sample should be representative of the population, and give accurate estimates of its characteristics. But pure randomness is difficult to achieve, especially when the sample size is small.  

Below we generate a random sample of 10 observations, from a sample space comprising integers from 1 to 6 (like rolling a die 10 times):^[You can generate random samples in R with the `sample()` function. You must specify `x` (the sample space), `size` (the sample size), and `replace` (a logical indicating whether to sample with replacement).]  
...
If we have defined $X$ as the RV for the outcome of a dice roll, the above output is a random sample of 10 observations of $X$. The sample mean, $\bar X$, is:  
...
Compare these sample means to the theoretical mean of the population, in this case $\mu = 3.5$. Each sample mean has an error that varies substantially across the two observed samples.^[Here error refers to the difference between the true and sample values of a parameter.]

Of course, this a trivial example, since the RV in question follows a known distribution (a discrete uniform distribution), and we *know* the population mean exactly. In most experimental contexts the population distribution is unknown---all we have is a sample distribution, and whatever estimates it produces.   

## Two sources of error in an estimator

There are two main sources of error in sample-derived estimates:

- **sampling error**---the random the tendency of an estimate to vary across different samples  
- **bias**---the systematic tendency of an estimate to over/underestimate the true population parameter  

The random sampling error is a result of **variability**---the tendency of data to vary randomly across different samples, thereby producing different results. In sections 10--3 to 10--5, we introduce two convergence theorems---both crowning achivements in statistics---that describe how the variability of an estimate depends on sample size.  

In this module (II) we will focus on concepts related to variability. In module III we will look at sources of bias.  

## The reliability of an estimator 

Reliability describes the tendency of an estimator to deliver _consistent_ results across samples. An estimator with high variability---e.g. the sample mean in the "diceroll" simulation above---is said to be *unreliable*, as its value will tend to change significantly across samples.  
...
The law of large numbers describes how the sample mean evolves as sample size increases.  

The "diceroll" simulation above used a sample size of $n = 10$. Below we rerun the same simulation three times, but with sample sizes $n = \{ 30, 1000, 10000 \}$. The sample mean is computed for each value of $n$.  
...
Predictably, increasing sample size brings the sample mean closer to its theoretical value, $\mu=3.5$. This is, in essence, the **law of large numbers**---it describes how the sample mean converges to its true value as sample size gets large.  
...
The **weak law of large numbers** says the sample mean converges *in probability* to the true mean for large $n$. Specifically, it say the probability the absolute difference between the sample mean and the true mean is larger than some nonzero margin goes to zero as the sample size increases: 

$$\P( | \bar X - \mu | > c) \rightarrow 0 \hspace{1cm} n \rightarrow \infty$$
This may be a cumbersome definition---but all it really says is the sample mean $\bar X$ is *likely* to be near $\mu$ when $n$ is large. Crucially, the weak LLN leaves open the possibility that $| \bar X - \mu | > c$ at infrequent intervals, i.e. that *occasionally* the sample mean might deviate from the true mean, even with very large $n$. 
...
In the previous example, we drew a random sample from a known distribution, yielding some sample mean. When we repeated the process, we got a different sample mean. We could, of course, do this repeatedly---below, the same experiment (i.e. a die being rolled 30 times) is repeated 10 times, each time with a new random sample. The 10 sample means are stored in a vector and printed below:  
...
As it turns out, we can model the sample mean $\bar X$ as a random variable (provided we have a collection of sample means), which gives a **sampling distribution**. Below is the distribution of the 10 sample means calculated above: 
...
This is known as the **sampling distribution of the mean**: it shows the distribution of the sample mean, $\bar X$, across several samples. Each sample has size $n=30$ (i.e. 30 independent observations of $X$).   

As we repeat the experiment with more and more random samples, it turns out the shape of the sampling distribution converges. The plots below show the sampling distributions of $\bar X$ for 100, 1000, and 10,000 random samples respectively, each with $n=30$. Summary statistics for each distribution are shown below. 
...
This is an important result in statistics, as it implies the normal distribution can be used to model many random processes, even those that intrinsically follow different distributions.  
...
It turns out the s.d. of a sample distribution is scaled down from the s.d. of the population distribution by a factor of $\sqrt n$:
...
Below we plot three sampling distributions, with sample sizes $n=\{ 10,50,100 \}$. The underlying RV is the same as above---a discrete uniform distribution with values from 1 to 6.  
...
In this chapter we've introduced two important results in statistics. Both concern the convergence in the behavior of random variables under large sample sizes. This part of probability is known as **large sample theory** or **asymptotic theory**. 
...
Since statistics is all about gathering large amounts of data and making inferences, these theorems are immensely useful for characterizing sample variability, and they have many applications in the statistical methods we introduced in module I.  

Below are two important caveats you ought to be aware of:
...
You may have noticed that at first, increasing the sample size makes a significant change to the distribution, rapidly bringing the sample mean closer to the true mean, and rapidly reducing the variance. But as $n$ gets larger, it takes longer for the sample mean to get closer to the true value by the same amount---this is known as the **law of diminishing information**. 
...
The above plot also depicts this: the dispersion of the distribution reduces substantially when $n$ is increased from 10 to 50, but change is more marginal as $n$ is increased from 50 to 100.  
