...
Just as the sample mean is a point estimator for the true mean, a 95\% confidence interval for the mean is an interval estimator for the true mean, and can be expressed as follows: 
...
Theoretically you can compute a confidence interval for any parameter, e.g. the median, max, min, etc. Mostly in statistics we're interested in the mean, and so in this section we will show how to compute a confidence interval for a mean.  
...
Since the mean follows a normal distribution, you can visualize a 95\% confidence interval for the mean as follows:  
...
i.e. if the mean follows a $t$-distribution, a 95\% confidence interval will have a lower and upper bound 2.04 standard errors from the mean.  
...
Suppose we want to compute a 95\% confidence interval for the true mean difference in percentage hourly wages. Since $n > 30$, we can assume the sample mean is approximately normal with:
...
where we have used the CLT with sample values as plug-in estimates. The general form of the confidence interval as follows:
...
This interval is only trivially different to the one computed using the normal, since $n$ in this case is large. 

But if, for instance, we had used a $t$-distribution with only 10 DoF, note how $c$ is more substantially different: 
...
i.e. when $n$ is large enough, it's sufficient to use the normal distribution to compute confidence intervals (with the appropriate plug-in estimates). But when dealing with small samples, you must use the $t$-distribution. 

## A function for calculating confidence intervals 

Currently there is no function in R to compute a confidence interval for a given array of data. You either have to perform the computations manually (as demonstrated above) or write a function yourself.   
...
Running this code will define a new function in the environment, `confidence_interval()`, that takes two arguments: `data`, a vector array of numeric data, and `conflevel`, the desired confidence level. It then computes a confidence interval for the mean using the desired confidence level.  

You can now use this function to compute a 95\% confidence interval for the mean difference in hourly wages:  
...
_**A a 95\% confidence interval is a range of values where, if you repeated the experiment many times, approximately 95\% of the confidence intervals generated will contain the true parameter.**_  
...
Nontheless, confidence intervals are still useful in estimation, since they provide wider range of plausible values for the true parameter than a point estimate does.   
