...
# 18--1 The Mean Squared Error

How good is an estimator overall? We know that there are two sources of error in an estimator: the error due to *variance*, and the error due to *bias:*

The **variance** of an estimator gives the average squared deviation from the expected value:

$$\Var[\hat\theta] = \E \big[ (\hat\theta - \E[\hat\theta])^2 \big]$$

The **bias** of an estimator is how far the expected value is from the true parameter:

$$\Bias[\hat\theta] = \E[\hat\theta] - \theta$$

Note the difference between variance and bias: variance is a measure of the *spread* of an estimate (how closely the data points are clustered), and bias is a measure of how far the cluster actually is from the true value. Below is a graphical illustration of different combinations of bias and variance:  

```{r, echo=FALSE, out.height=400, out.width=400, fig.align='center'}
knitr::include_graphics('./pics/c18-pic1.png')
```

The **mean squared error** provides a way to quantify both sources of error. The MSE is defined as the expected value of the squared errors:

$$\MSE[\hat\theta] = E\big[ (\hat\theta - \theta)^2 \big]$$

This gives a measure of how far, on average, a collection of estimates are from the true parameter being estimated.  

Note if we expand the formula for MSE we get:

$$\MSE[\hat\theta] = (\E[\hat\theta] = \theta)^2 + \E \big[ (\hat\theta - \E[\hat\theta])^2 \big]$$

$$\MSE[\hat\theta] = \Bias[\hat\theta]^2 + \Var[\hat\theta]$$

i.e. mean squared error is equal to the bias squared plus the variance.  
