...
In a hypothesis test, the null assumed until there is strong evidence to suggest we should reject it. Of course, the outcome of a single hypothesis test does not necessarily lead to the correct result---the $p$-value (or whatever rubric you use) only represents a *likelihood*, and there's always a chance the conclusion we draw from a given test is the *wrong* conclusion, even if the likelihood is very low.   
...
*Example*---to visualize the type I and type II error, consider the case where the null hypothesis is $H_0: \mu = 0$. Suppose also that $\sigma = 1$ and $n = 100$. The figure below shows the distribution under the null, with the rejection region for an $\alpha = 0.05$ test shaded. If the null is *actually* true, then the probability of rejecting the null is simply the rejection region:  

```{r, echo=FALSE, warning=FALSE, fig.height=3.5, fig.width=5, fig.align='center'}
mu = 0; sigma = 1; n = 100; SE = sigma/sqrt(n)
...
breaks = round(seq(-0.4, 0.4, 0.1),3)

ggplot(data = df, mapping = aes(x = x, y = y)) +
  geom_line() +
  scale_x_continuous(breaks = breaks, 
                     limits = c(-0.4, 0.4)) +
  ylab('density') + xlab(TeX('$\\bar{X}$')) + 
  geom_area(aes(x = ifelse(x < mu - c_null*SE, x, mu - c_null*SE)), fill='black', alpha=0.4) +
  geom_area(aes(x = ifelse(x > mu + c_null*SE, x, mu + c_null*SE)), fill='black', alpha=0.4) +
  geom_segment(x = 0, xend = 0, y = -1, yend = 5, linetype = 'dotted') +
  geom_text(x = 0.05, y = 1, label = TeX('$H_0: \\; \\mu = 0$')) +
  geom_text(x = 0.25, y = -0.15, label = 'Type I Error', color = 'black') +
  geom_text(x = -0.25, y = -0.15, label = 'Type I Error', color = 'black') +
  ylim(-0.5,4.2) +
  theme_light()
```

Now suppose the true mean is actually $\mu = 0.3$. The figure below shows the distribution under the true mean, overlaid in pink. The type II error---the probability of failing to reject a false null---is the region to the left of the critical value, under the *true* distribution: 
...
y = dnorm(x, mean = mu, sd = SE)
x1 = seq(-0.5, 1, length = 1000)
y1 = dnorm(x, mean = 0.3, sd = SE)

df = data.frame(x = x, y = y, x1 = x1, y1 = y1)

breaks = round(seq(-0.4, 1, 0.1),3)
...
ggplot(data = df, mapping = aes(x = x1, y = y1)) +
  ...
  scale_x_continuous(breaks = breaks, 
                     limits = c(-0.4, 0.7)) +
  ylab('density') + xlab(TeX('$\\bar{X}$')) + 
  geom_area(aes(x = ifelse(x1 < mu + c_null*SE, x1, mu + c_null*SE)), fill='violetred', alpha=0.4) +
  geom_text(x = 0.05, y = 1, label = TeX('$H_0: \\; \\mu = 0$')) +
  geom_segment(x = 0.3, xend = 0.3, y = -1, yend = 5, color = 'violetred', linetype = 'dotted') +
  geom_text(x = 0.05, y = 1, label = TeX('$H_0: \\; \\mu = 0$')) +
  geom_text(x = 0.35, y = 1, label = TeX('true mean: $\\mu = 0.3$'), color = 'violetred') +
  geom_text(x = 0.25, y = -0.15, label = 'Type I Error', color = 'black') +
  geom_text(x = -0.25, y = -0.15, label = 'Type I Error', color = 'black') +
  geom_text(x = 0.08, y = -0.15, label = 'Type II Error', color = 'violetred') +
  ylim(-0.5,4.2) +
  theme_light() 

ggsave('./pics/c13-pic1.png')
```

```{r, echo=FALSE, fig.height=3.5, fig.width=5}
knitr::include_graphics('./pics/c13-pic1.png')
```

From the above figure you should be able to see why decreasing the likelihood of type I error (by increasing the significance level) causes the type II error to increase.  
...
ggplot(data = df, mapping = aes(x = x1, y = y1)) +
  ...
  ylab('density') + xlab(TeX('$\\bar{X}$')) + 
  geom_area(aes(x = ifelse(x1 < mu + c_null*SE, x1, mu + c_null*SE)), fill='violetred', alpha=0.4) +
  geom_area(aes(x = ifelse(x1 > mu + c_null*SE, x1, mu + c_null*SE)), fill='blue', alpha=0.4) +
  geom_text(x = 0.05, y = 1, label = TeX('$H_0: \\; \\mu = 0$')) +
  geom_segment(x = 0.3, xend = 0.3, y = -1, yend = 5, color = 'violetred', linetype = 'dotted') +
  geom_text(x = 0.35, y = 1, label = TeX('true mean: $\\mu = 0.3$'), color = 'violetred') +
  geom_text(x = 0.08, y = -0.15, label = 'Type II Error', color = 'violetred') +
  geom_text(x = 0.5, y = 2, label = 'Power', color = 'blue') +
  ylim(-0.5,4.2) +
  theme_light() 
```

The further away the true mean is from the proposed mean, the higher the power of the test. To see this, we'll consider something called the **power function**, $\beta$,^[Not to be confused with the regression coefficient, which is also denoted $\beta$.] which describes the power of a test as a function of $\mu$, i.e. $\beta = \beta(\mu)$. Since power is the probability of correctly rejecting the null, the power function is:

$$
\begin{aligned}
  \beta(\mu) &= \P(\bar X > c) \\ 
  &= \P \bigg( \frac{\sqrt n (\bar X - \mu)}{\sigma} > \frac{\sqrt n (c - \mu)}{\sigma} \bigg) \\ 
  &= \P \bigg( Z >  \frac{\sqrt n (c - \mu)}{\sigma} \bigg) \\ 
  &= 1 -\P \bigg( Z < \frac{\sqrt n (c - \mu)}{\sigma} \bigg) \\ 
  &= 1 - \Phi \bigg(  \frac{\sqrt n (c - \mu)}{\sigma} \bigg)
\end{aligned}
$$

where $\Phi$ is the cdf of the normal distribution. This function is clearly increasing in $\mu$, which means the power ($\beta$) increases as the true mean gets further away from its value under the null. Below is a plot of power as a function of the true mean:  

```{r, echo=FALSE, warning=FALSE, fig.height=3.5, fig.width=5, fig.align='center'}
c_null = qnorm(0.975)

power = function(mu) {
  1- pnorm(c_null - 10*mu)
}

xrange = data.frame(mu = seq(from = -0.5, to = 0.5, length = 100))

ggplot(xrange, aes(mu)) + 
  stat_function(fun = power) +
  xlab(TeX('$\\mu$')) + ylab(TeX('$\\beta(\\mu)$')) +
  ggtitle('power as a function of true mean') +
  xlim(0,0.5) +
  theme_light()
```
...
power = function(mu) {
  pnorm(-c_null - 10*mu) + pnorm(c_null - 10*mu, lower.tail = F)
}
...
ggplot(xrange, aes(mu)) + 
  ...
  geom_segment(x = -0.6, xend = 0, y = 0.05, yend = 0.05, color  = 'violetred', linetype = 'dotted') +
  geom_text(x = -0.3, y = 0.1, label = TeX('$\\alpha = 0.05$'), color = 'violetred') + 
  theme_light()
```
