...
`r margin_note("Note the $\\texttt{chisq.test()}$ function requires data to be in the matrix or table format.")`
...
Note in the above example we tested several hypotheses simultaneously---we could also have tested each group (advanced ailment and minimal ailment) *separately* against the control group for dependence on blood type:
...
Note how each of these groups, when tested *separately* against the control group, yield vastly different results: the advanced ailment group has a significant $p$-value of 0.00343, but the minimal ailment group does not have a significant $p$-value.   
...
Testing many hypotheses simultaneously is known as *multiple testing*. For any one test, the chance of a false rejection of the null (type I error) is $\alpha$. But when conducting many tests simultaneously, the chance of *at least* one false rejection is much larger:

$$\P(\text{at least 1 false rejection in }m \text{ tests}) = 1  - \P(\text{no false rejections in }m \text{ tests}) = 1-(1-\alpha)^m$$

This is the **multiple testing problem**---it becomes particularly problematic when we conduct a very large number (e.g. thousands or millions) of hypothesis tests simultaneously. Note the probability of making at least 1 false rejection (or type I error) is known as the **familywise error rate**, or FWER.   

Below we will discuss two methods of dealing with this problem: the **Bonferonni** method and the **Benjamini-Hochberg** method. 

## The Bonferroni method

The Bonferonni method gives a way to control the familywise error rate (FWER). For $m$ tests, the Bonferonni method works as follows: 

$$H_{0i} \;\; \text{vs.} \;\; H_{1i} \;\;\;\; \text{for} \;\; i = 1,...,m$$

If $p_1, ..., p_m$ denote the $m$ $p$-values for these tests, then reject the null hypothesis $H_{0i}$ if and only if: 

$$p_i < \frac \alpha m$$

This condition controls the familywise error rate to be $\text{FWER} \leq \alpha$. To see this:

$$\text{FWER} = \P \Bigg\{ \bigcup_{i=1}^m \bigg( p_i \leq \frac \alpha m \bigg) \Bigg\} \leq \sum_i^m \bigg\{ \P(p_i \leq \frac \alpha m \bigg) \bigg\} = \sum_i^m \frac \alpha m = \alpha$$

In R, you can adjust a vector of $p$-values with the Bonferonni correction with the function `p.adjust(p, method = p.adjust.methods = 'bonferonni')`, where `p` is a numeric vector of $p$-values.    
...
For $m$ tests, the BH method works as follows: 

- arrange the observed $p$-values $p_1, ..., p_m$ in increasing order, and assign ranks to each value based on its order, i.e. $p_{(1)}, p_{(2)}, ...$
- for each $p$-value, compute the BH critical value, defined as $\frac im \alpha$, where $\alpha$ is the desired false discovery rate (e.g. 5\% or 10\%)
