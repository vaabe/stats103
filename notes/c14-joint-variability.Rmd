...
Sometimes this is known as the **univariate** normal distribution, since it's a function of one random variable, $X$. We've seen how useful the univariate normal is for modeling random variables and describing their asymptotic properties.  

Often in statistics we must deal with a *combination* of random variables (e.g. in multiple regression). These situations often involve the **multivariate normal distribution**, or **joint normal**---this is a generalization of the simple normal distribution to higher dimensions.  
...
Below is a plot showing the bivariate normal and heatmap for a random vector $\v X$ with mean vector $\v \mu = \big[\begin{smallmatrix} 2 \\ 1 \end{smallmatrix} \big]$ and covariance matrix $\v \Sigma = \big[ \begin{smallmatrix} 1 \\ 0.4 \end{smallmatrix} \begin{smallmatrix} 0.9 \\ 1.5 \end{smallmatrix} \big]$ (i.e. $X_1$ and $X_2$ are *correlated*):
