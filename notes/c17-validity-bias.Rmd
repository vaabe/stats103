...
If we have a random unbiased sample from a population with true mean $\mu$, and $X$ is a RV for the observed outcomes in the sample, then we should expect that $\E[X]=\mu$, and that the larger the sample, the better the approximation.  

But if the sampling method is biased, and certain segments of the population are excluded/underrepresented, the expected value of $X$ is no longer the population mean, but rather the mean of the biased subset, i.e. $\E[X] = c \neq \mu$. In this case increasing $n$ will make $\bar X$ converge to $c$, not $\mu$.  
...
Since confidence intervals are centered on the sample mean, $\bar X$, the probability that a given confidence interval "captures" the true mean depends on how close $\bar X$ is to $\mu$. If the sample is biased, confidence intervals will be centered somewhere other than $\mu$, and the probability of capturing the true mean will converge to zero as $n$ increases.  

E.g. suppose a population has true mean $\mu = 10$. If our sample is biased, such that $\E[X] = 12 \neq \mu$, a 95\% confidence interval computed on $\bar X$ will be centered at 12, not 10. With $s = 6$ and $n=20$, a 95\% confidence interval covers the following region:
...
i.e. with $n=20$ the asymptotic 95\% confidence interval contains $\mu$, but only just. 
...
The takeaway---while increasing $n$ reduces the variability of an estimate, if the sample is biased it will decrease the "coverage" probability of the interval. As $n$ gets large, the probability the interval contains $\mu$ will asymptotically converge to zero---we will narrow in on a range of values that no longer contains the true parameter value.   
...
ggplot(data = df1, mapping = aes(x = x, y = y)) +
  ...
  geom_text(label = TeX('$H_0: \\; \\mu = 12$'), x = Xbar, y = 0.12, size = 5, color = 'black') +
  geom_segment(x = mu, xend = mu, y = 0, yend = 0.5, color = 'violetred', linetype = 'dotted') +
  geom_text(label = TeX('$\\mu = 9$'), x = mu, y = 0.2, size = 5, color = 'violetred') +
  geom_text(label = TeX('$\\bar{X}$'), x = Xbar, y = 0.2, size = 5, color = 'black') +
  theme_bw()
```


But suppose now our sample is biased, such that $\E[X] = 10.5$, i.e. the bias makes the sample mean *closer* to the null:
...
ggplot(data = df2, mapping = aes(x = x, y = y)) +
  ...
  geom_text(label = TeX('$\\mu = 9$'), x = mu, y = 0.2, size = 5, color = 'violetred') +
  geom_text(label = TeX('$\\bar{X}$'), x = Xbar_bias, y = 0.25, size = 5, color = 'darkcyan') +
  theme_bw()
```

In this case the bias makes the sample mean closer to the null, which *reduces* the power of the test.  

Conversely, if the sample bias is such that $\E[X] = 7.5$, i.e. the bias makes the sample mean further from the null:
...
ggplot(data = df2, mapping = aes(x = x, y = y)) +
  ...
  geom_segment(x = Xbar - Z_5*SE, xend = Xbar - Z_5*SE, y = 0, yend = 0.1, color = 'black', linetype = 'dotted') + 
  geom_segment(x = Xbar + Z_5*SE, xend = Xbar + Z_5*SE, y = 0, yend = 0.1, color = 'black', linetype = 'dotted') +
  geom_text(label = 'Type II Error', x = 11, y = -0.005, size = 5, color = 'darkcyan') +
  geom_text(label = 'Power', x = 7.1, y = -0.005, size = 5, color = 'blue') +
  geom_text(label = 'critical \n value', x = Xbar - Z_5*SE, y = 0.12, size = 3, color = 'black') +
  geom_text(label = 'critical \n value', x = Xbar + Z_5*SE, y = 0.12, size = 3, color = 'black') +
  geom_segment(x = Xbar, xend = Xbar, y = 0, yend = 0.5, color = 'black', linetype = 'dotted') +
  geom_text(label = TeX('$H_0: \\; \\mu = 12$'), x = Xbar, y = 0.12, size = 5, color = 'black') +
...
In this case the bias makes the sample mean further to the null, which *increases* the power of the test.   

The takeaway---a biased sample can increase or decrease the power of the test, depending on whether the bias brings the sample mean nearer or further to the true mean.  
