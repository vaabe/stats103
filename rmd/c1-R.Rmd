...
In general you can call a function on an object (or a set of objects) by typing the function name followed by a pair of brackets. In the brackets you specify the object(s) you want the function to operate on:

```
function_name(object1, object2, ...)
```

What you type in the brackets are known as **arguments** or **inputs** to the function. Most functions accept several arguments--these are not only for specifying objects, but also for specifying **function parameters** to make the function behave in a certain way. 

E.g. if you want to compute the mean of an array that has missing values, you'll find the `mean()` function in its default setting doesn't quite work:
...
However if you specify the optional parameter `na.rm = TRUE`, this forces the function to skip all `NA` values: 

```{r}
mean(some_numbers, na.rm = TRUE)
```

If you leave optional arguments blank when calling a function, it will operate according to its default settings. 
...
After choosing your output format, the editor will open and you will see some sample Markdown script that looks like this: 
